...
class BookOrder(models.Model):
    ...
    login_name = models.ForeignKey(Customer, on_delete=models.RESTRICT)
    isbn = models.ForeignKey(Book, on_delete=models.RESTRICT)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['order_number', 'login_name', 'isbn'], name='unique item in order')]
...
class Comment(models.Model):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=['login_name', 'isbn'], name='unique comment')]

    def __str__(self):
        return f'login name:{self.login_name}\nisbn:{self.isbn}'
...
class Offense(models.Model):
    ...
    def __str__(self):
        return f'login_name:{self.login_name}\ncount:{self.offense_count}'
...
class TrustedCustomer(models.Model):
    login_name = models.ForeignKey(Customer, on_delete=models.RESTRICT, related_name='t_login_name')
    trusted_login_name = models.ForeignKey(Customer, on_delete=models.RESTRICT, related_name='t_trusted_login_name')
...
class UnTrustedCustomer(models.Model):
    login_name = models.ForeignKey(Customer, on_delete=models.RESTRICT, related_name='ut_login_name')
    untrusted_login_name = models.ForeignKey(Customer, on_delete=models.RESTRICT,
                                             related_name='ut_untrusted_login_name')
